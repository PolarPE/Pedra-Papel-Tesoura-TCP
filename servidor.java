...
public class servidor {
    ...
    private void start() throws IOException {
        ...
        while (running) {
            ...
            String choicesList = "";
            for (Player p : players) choicesList += "P" + p.id + ":" + moves.get(p.id) + " ";
            broadcast(players, "ESTADO " + choicesList.trim());

            if (winners.isEmpty()) { broadcast(players, "MENSAGEM Resultado: Empate"); broadcast(players, "EMPATE"); }
            else {
                for (Player p : players) { if (winners.contains(p.id)) p.sendMessage("VENCEU"); else p.sendMessage("PERDEU"); }
                broadcast(players, "MENSAGEM Vencedor(es): " + winners.toString());
            }
    ...
    private boolean waitForAllConfirm(List<Player> players) {
        ...
        while (confirmed.size() < players.size()) {
            try {
                ...
                System.out.println("DEBUG(wfc): recebido de " + m.playerId + ": '" + m.text + "'");
    ...
    private boolean waitForAllRematch(List<Player> players) {
        ...
        while (agreed.size() < players.size()) {
            try {
                ...
                System.out.println("DEBUG(wfr): recebido de " + m.playerId + ": '" + m.text + "'");
    ...
    private static void sendEstado(List<Player> players, Map<Integer, String> moves) { StringBuilder sb = new StringBuilder(); for (Player p : players) sb.append("P").append(p.id).append(":").append(moves.getOrDefault(p.id, "-")).append(" "); for (Player p : players) p.sendMessage("ESTADO " + sb.toString().trim()); }
