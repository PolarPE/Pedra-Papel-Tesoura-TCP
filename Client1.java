...
public class Client1 {
	...
	public static void main(String[] args) {
		...
		if (System.console() != null) {
			...
			// entrada redirecionada — mantém comportamento automático para scripts
			runClient(host, port);
		}
	}
		...
		try (Socket socket = new Socket(host, port);
			...
			Thread inputThread = new Thread(() -> {
				try {
					...
					while ((uline = console.readLine()) != null) {
						...
						else toSend = t; // envia texto cru (ex: S para confirmar, REMATCH SIM, etc)
			...
			while ((line = in.readLine()) != null) {
				if (line.startsWith("ASSIGN ") || line.startsWith("ENTROU ")) {
					...
					System.out.println("Sua vez: escolha ROCK, PAPER ou TESOURA (R/P/S): (pode digitar a qualquer momento)");
					// entrada será lida pela thread de stdin
